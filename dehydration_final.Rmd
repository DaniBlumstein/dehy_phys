...
all_noOL_clean <- all_noOL %>% 
  separate(StartTime, into = c("Hour", "Minute", "Second"), sep = ":") %>% 
  mutate(Day = day - 1) %>%
  mutate(time_period = period(days = Day, hours = Hour, minutes = Minute, seconds = Second)) %>%
  mutate(Hours = as.numeric(time_period)/3600) %>%
  mutate(Date = as.POSIXct("2021-01-01") + time_period) %>%
  select(Date, Day, Hour, Minute, Second, RQ, Sex, EE_kJH, H2Omg, tt)
...
#scale_x_datetime(date_labels = "Day %d \n Hour: %H", date_breaks = "6 hours") +

H2Omg_plot <- ggplot(data = all_noOL_clean,aes(color = tt, x=Date,y=H2Og))+
  annotate("rect", xmin = as.POSIXct("2021-01-01 21:00:00", tz="UTC"),
           xmax = as.POSIXct("2021-01-02 06:00:00", tz="UTC"),
           ymin = -Inf, ymax = Inf, fill="grey84") +
  annotate("rect", xmin = as.POSIXct("2021-01-02 21:00:00", tz="UTC"),
           xmax = as.POSIXct("2021-01-03 06:00:00", tz="UTC"),ymin = -Inf, 
           ymax = Inf, fill="grey84") +
  annotate("rect", xmin = as.POSIXct("2021-01-03 21:00:00", tz="UTC"),
           xmax = as.POSIXct("2021-01-04 06:00:00", tz="UTC"),ymin = -Inf, 
           ymax = Inf, fill="grey84") +
  scale_x_datetime(date_labels = "%H", date_breaks = "4 hours") +
  scale_color_manual(values=cols, name = "Treatments") +
  geom_point(alpha = .5)+
  theme_classic()+
  #geom_smooth(method='loess', span=.1, level=0.99)+
...
```{r}
M2_v1 <- bam(H2Og ~ H2O * Sex + s(days, by=tt, bs='cc',k=20)  + s(time_in_D,by=tt, bs="cr",k=40), data = dd, random = list(startexp = ~1, Animal_ID = ~1|startexp), method="REML")

M2_v2 <- bam(H2Og ~ H2O * Sex + s(days, time_in_D, by=tt,k=70), data = dd, random = list(startexp = ~1, Animal_ID = ~1|startexp), method="REML")
